...
class CLS_CONTENTS{
    ...
    public function getList($strwhere="",$limit=''){
        $sql="SELECT * FROM tbl_contents WHERE isactive=1 $strwhere $limit";
        return $this->objmysql->Query($sql);
    }
    public function getCount($where=''){
        $objdata=new CLS_MYSQL;
        $sql="SELECT count(id) as count FROM `tbl_contents` WHERE isactive=1 ".$where;
        $objdata->Query($sql);
    ...
    public function updateView($code){
        if(!isset($_SESSION['VIEW-CONTENT'])){
            $sql="UPDATE `tbl_contents` SET `visited`=`visited`+1 WHERE `code` ='$code'";
