...
class CLS_QUESTION_GROUP{
    ...
    public function getList($where='',$limit=''){
        $sql="SELECT * FROM `tbl_question_group` WHERE isactive=1 ".$where.$limit;
        return $this->objmysql->Query($sql);
    }
    public function getInfo($where=''){
        $sql="SELECT * FROM `tbl_question_group` WHERE isactive=1 ".$where;
    ...
    public function getCatIDParent($parid){
        $sql="SELECT * FROM `tbl_question_group` WHERE isactive=1 AND cate_id='$parid' ";
        $objdata=new CLS_MYSQL();
