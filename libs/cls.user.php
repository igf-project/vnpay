...
class CLS_USER{
	...
	public function permission(){
		if($_SESSION[MD5($_SERVER['HTTP_HOST']).'_USERLOGIN']['gid']==1 || $_SESSION[MD5($_SERVER['HTTP_HOST']).'_USERLOGIN']['isroot']==1) return true;
	}
	...
	//-------------------------------------------------------
	public function getGroupUser($level){
		...
		if($objdata->Num_rows()){
			...
			while($r=$objdata->Fetch_Assoc()){
				...
				$name=$r["name"];
				if($r["isroot"]==0)
					echo "<li><a href='javascript:void(0);' onclick='user_group_select(this);' dataid='$id'>$name</a>";
				else
					echo "<li class='disable' disabled><a href='javascript:void(0);'>$name</a>";
				$next_level=$level+1;
				echo "</li>";
			}
			echo "</ul>";
