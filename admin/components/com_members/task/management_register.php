<?php
$strwhere='';$cur_page=0;$class='col-md-9';
...
<div class="body">
	...
	<script>
		...
		$(document).ready(function(){
			...
			$('.cmd_group_permission a').click(function(){
				var _gid=$('#guser_selected').val();
				if(_gid=='' || _gid==0) showMess('Bạn chưa chọn nhóm để phân quyền','error');
				else{
					$('#myModalPopup .modal-dialog').removeClass('modal-sm');
					$('#myModalPopup .modal-dialog').addClass('modal-lg');
					$('#myModalPopup .modal-header .modal-title').html('Phân quyền nhóm người dùng');
					$('#myModalPopup .modal-body').html('<p>Loadding...</p>');
					var url='ajaxs/user/frm_add_permission_group.php'; 
					$.get(url,{'id':_gid},function(req){
						if(req=='E01'){showMess('Bạn chưa đăng nhập, xin vui lòng đăng nhập!','error');}
						if(req=='E02'){showMess('Không có quyền phân quyền nhóm này!','error');}
						else{
							$('#myModalPopup .modal-body').html(req);
							$('#myModalPopup').modal('show');
						}
