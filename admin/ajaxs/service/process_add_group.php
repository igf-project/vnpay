...
if($id>0){
	$sql="UPDATE `tbl_cate_service` SET `par_id`='$par_id',`name`='$name',`code`='$code',`thumb`='$filename',`intro`='$intro' WHERE cate_id='$id'";
	$sql=$objdata->Exec($sql);
}else{
	$sql="INSERT INTO tbl_cate_service (`par_id`,`name`,`code`,`thumb`,`intro`,`isactive`) 
