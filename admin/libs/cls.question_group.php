...
class CLS_QUESTION_GROUP{
    ...
    public function getInfo($where='',$limit=''){
        $sql="SELECT * FROM `tbl_question_group` WHERE 1=1 ".$where.' ORDER BY `name` '.$limit;
    ...
    public function listTable($strwhere="",$page=1,$parid=0,$level=0,$rowcount){
        ...
        $leng=MAX_ROWS;
        $sql="SELECT * FROM tbl_question_group WHERE 1=1 $strwhere AND par_id=$parid ORDER BY `order` ASC LIMIT $star,$leng";
        $objdata=new CLS_MYSQL();
        $objdata->Query($sql);
        $str_space="";
        if($level!=0){  
    ...
    public function getNameById($id){
        $objdata=new CLS_MYSQL;
        $sql="SELECT `name` FROM `tbl_question_group`  WHERE isactive=1 AND `cate_id` = '$id'";
        $objdata->Query($sql);
        $row=$objdata->Fetch_Assoc();
        return $row['name'];
    }
    public function Add_new(){
        $sql=" INSERT INTO `tbl_question_group`(`par_id`,`name`,`code`,`thumb`,`intro`,`type`,`isactive`) VALUES";
    ...
    public function Delete($id){
        $sql="DELETE FROM `tbl_question_group` WHERE `cate_id` in ('$id')";
        return $this->objmysql->Exec($sql);
    }
    public function setActive($ids,$status=''){
        $sql="UPDATE `tbl_question_group` SET `isactive`='$status' WHERE `cate_id` in ('$ids')";
        if($status=='')
            $sql="UPDATE `tbl_question_group` SET `isactive`=if(`isactive`=1,0,1) WHERE `cate_id` in ('$ids')";
        return $this->objmysql->Exec($sql);
    }
    public function Order($arr_id,$arr_quan){
        $n=count($arr_id);
        for($i=0;$i<$n;$i++){
            $sql="UPDATE `tbl_question_group` SET `order`='".$arr_quan[$i]."' WHERE `cate_id` = '".$arr_id[$i]."' ";
            $this->objmysql->Exec($sql);
        }
    }
    /* combo box*/
    function getListCbItem($getId='', $swhere=''){
        $sql="SELECT cate_id, name, code FROM tbl_question_group ".$swhere." ORDER BY `name` ASC";
        $objdata=new CLS_MYSQL();
        $objdata->Query($sql);
        if($objdata->Num_rows()<=0) return;
        while($rows=$objdata->Fetch_Assoc()){
    ...
    public function getListCategory($parid=0,$level=0){
        $sql="SELECT * FROM `tbl_question_group` WHERE `par_id`='$parid' AND `isactive`=1 ORDER BY `order` ASC";
        $objdata=new CLS_MYSQL();
        $objdata->Query($sql);
        $char="";
        if($level!=0){
        ...
        if($objdata->Num_rows()>0){
            ...
            while($r=$objdata->Fetch_Assoc()){
                $id=$r["cate_id"];
                $name=$r["name"];
                echo"<li data-id='$id'><a href='".ROOTHOST_ADMIN."question/question_group/$id'>$char $name</a></li>";
                $nextlevel=$level+1;
