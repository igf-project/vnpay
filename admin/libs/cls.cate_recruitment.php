...
class CLS_CATEGORY_RECRUITMENT{
    ...
    public function getList($where='',$limit=''){
        $sql="SELECT * FROM `tbl_cate_recruitment` ".$where.' ORDER BY `name` '.$limit;
        return $this->objmysql->Query($sql);
    }
    public function getInfo($where='',$limit=''){
        $sql="SELECT * FROM `tbl_cate_recruitment` WHERE 1=1 ".$where.' ORDER BY `name` '.$limit;
    ...
    public function listTable($strwhere="",$page=1,$parid=0,$level=0,$rowcount){
        ...
        $leng=MAX_ROWS;
        $sql="SELECT * FROM tbl_cate_recruitment WHERE 1=1 $strwhere AND par_id=$parid ORDER BY `order` ASC LIMIT $star,$leng";
        $objdata=new CLS_MYSQL();
        $objdata->Query($sql);
        $str_space="";
        if($level!=0){  
    ...
    public function Add_new(){
        $sql=" INSERT INTO `tbl_cate_recruitment`(`par_id`,`name`,`code`,`thumb`,`intro`,`type`,`isactive`) VALUES";
        $sql.="('".$this->Par_Id."','".$this->Name."','".$this->Code."','".$this->Thumb."','".$this->Intro."','".$this->Type."','".$this->isActive."')";
        return $this->objmysql->Exec($sql);
    }
    public function Update(){
        $sql = "UPDATE tbl_cate_recruitment SET 
        `par_id`='".$this->Par_Id."',
        `name`='".$this->Name."',
        `code`='".$this->Code."',
        `thumb`='".$this->Thumb."',
    ...
    public function Delete($id){
        $sql="DELETE FROM `tbl_cate_recruitment` WHERE `cate_id` in ('$id')";
        return $this->objmysql->Exec($sql);
    }
    public function setActive($ids,$status=''){
        $sql="UPDATE `tbl_cate_recruitment` SET `isactive`='$status' WHERE `cate_id` in ('$ids')";
        if($status=='')
            $sql="UPDATE `tbl_cate_recruitment` SET `isactive`=if(`isactive`=1,0,1) WHERE `cate_id` in ('$ids')";
        return $this->objmysql->Exec($sql);
    }
    public function Order($arr_id,$arr_quan){
        $n=count($arr_id);
        for($i=0;$i<$n;$i++){
            $sql="UPDATE `tbl_cate_recruitment` SET `order`='".$arr_quan[$i]."' WHERE `cate_id` = '".$arr_id[$i]."' ";
    ...
    public function getListCategory($parid=0,$level=0){
        $sql="SELECT * FROM `tbl_cate_recruitment` WHERE `par_id`='$parid' AND `isactive`=1 ORDER BY `order` ASC";
        $objdata=new CLS_MYSQL();
        $objdata->Query($sql);
        $char="";
        if($level!=0){
        ...
        if($objdata->Num_rows()>0){
            ...
            while($r=$objdata->Fetch_Assoc()){
                $id=$r["cate_id"];
                $name=$r["name"];
                echo"<li data-id='$id'><a href='".ROOTHOST_ADMIN."recruitment/cate_recruitment/$id'>$char $name</a></li>";
                $nextlevel=$level+1;
