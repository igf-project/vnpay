...
class CLS_CATEGORY{
    ...
    public function getNameById($id){
        $objdata=new CLS_MYSQL;
        $sql="SELECT `name` FROM `tbl_category`  WHERE isactive=1 AND `cate_id` = '$id'";
        $objdata->Query($sql);
        $row=$objdata->Fetch_Assoc();
        return $row['name'];
    }
    ...
    public function Delete($id){
        $sql="DELETE FROM `tbl_category` WHERE `cate_id` in ('$id')";
        return $this->objmysql->Exec($sql);
    }
    public function setActive($ids,$status=''){
        $sql="UPDATE `tbl_category` SET `isactive`='$status' WHERE `cate_id` in ('$ids')";
        if($status=='')
            $sql="UPDATE `tbl_category` SET `isactive`=if(`isactive`=1,0,1) WHERE `cate_id` in ('$ids')";
        return $this->objmysql->Exec($sql);
    }
    public function Order($arr_id,$arr_quan){
        $n=count($arr_id);
        for($i=0;$i<$n;$i++){
            $sql="UPDATE `tbl_category` SET `order`='".$arr_quan[$i]."' WHERE `cate_id` = '".$arr_id[$i]."' ";
            $this->objmysql->Exec($sql);
        }
    }
    /* combo box*/
    function getListCbItem($getId='', $swhere=''){
        $sql="SELECT cate_id, name, code FROM tbl_category ".$swhere." ORDER BY `name` ASC";
        $objdata=new CLS_MYSQL();
        $objdata->Query($sql);
        if($objdata->Num_rows()<=0) return;
        while($rows=$objdata->Fetch_Assoc()){
    ...
    public function getListCategory($parid=0,$level=0){
        $sql="SELECT * FROM `tbl_category` WHERE `par_id`='$parid' AND `isactive`=1 ORDER BY `order` ASC";
        $objdata=new CLS_MYSQL();
        $objdata->Query($sql);
        $char="";
        if($level!=0){
