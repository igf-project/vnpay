...
class CLS_QUESTION{
    ...
    public function __set($proname,$value){
        if(!isset($this->pro[$proname])){
            echo ($proname.' is not member of CLS_QUESTION Class' );
            return;
        }
    ...
    public function getList($strwhere=""){
        $sql="SELECT * FROM tbl_question WHERE 1=1 $strwhere";
        return $this->objmysql->Query($sql);
    }
    public function getCount($where=''){
        $objdata=new CLS_MYSQL;
        $sql="SELECT count(id) as count FROM `tbl_question` WHERE 1=1 ".$where;
        $objdata->Query($sql);
    ...
    public function LoadConten($lagid=0){
        $sql="SELECT * FROM `tbl_question` WHERE  isactive='1'";
        $objdata=new CLS_MYSQL();
        $objdata->Query($sql);
    ...
    public function getCatName($catid) {
        $sql="SELECT name FROM tbl_question_group WHERE cate_id='$catid'";
        $objdata=new CLS_MYSQL;
        $objdata->Query($sql);
    ...
    public  function getListCbRelateContent($getId='', $swhere='', $arrId=''){
        $sql="SELECT `id`,`title` FROM `tbl_question` WHERE ".$swhere." `isactive`='1' ORDER BY `id` ASC";
        $objdata=new CLS_MYSQL();
        $objdata->Query($sql);
        if($objdata->Num_rows()<=0) return;
        while($rows=$objdata->Fetch_Assoc()){
    ...
    public function listTable($strwhere="",$page){
        ...
        $leng=MAX_ROWS_ADMIN;
        $sql="SELECT * FROM tbl_question WHERE 1=1 $strwhere ORDER BY `id` DESC LIMIT $star,$leng";
        $objdata=new CLS_MYSQL();
    ...
    public function getOldArrListTags($con_id){
        $sql="SELECT * FROM `tbl_question` WHERE `id`=$con_id ";
        $arrTags=array();
        $objdata=new CLS_MYSQL();
        $objdata->Query($sql);
        $row=$objdata->Fetch_Assoc();
    ...
    public function addNew(){
        $objdata=new CLS_MYSQL;
        $sql="INSERT INTO tbl_question (`cate_id`,`code`,`thumb`,`list_tagid`,`list_conid`,`cdate`,`mdate`,`author`,`ishot`,`isactive`,`title`,`intro`,`fulltext`,`meta_title`,`meta_key`,`meta_desc`) VALUES ";
    ...
    function Order($arr_id,$arr_quan){
        $n=count($arr_id);
        for($i=0;$i<$n;$i++){
            $sql="UPDATE `tbl_question` SET `order`='".$arr_quan[$i]."' WHERE `id` = '".$arr_id[$i]."' ";
            $this->objmysql->Exec($sql);
        }
    }
    /* combo box*/
    function getListCbItem($getId='', $swhere='', $arrId=''){
        $sql="SELECT id, name, code FROM tbl_question WHERE ".$swhere." `isactive`='1' ORDER BY `name` ASC";
        $objdata=new CLS_MYSQL();
        $objdata->Query($sql);
        if($objdata->Num_rows()<=0) return;
        while($rows=$objdata->Fetch_Assoc()){
