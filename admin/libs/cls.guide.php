...
class CLS_GUIDE{
    private $pro=array(
        ...
        'isActive'=>'1');
    private $objmysql;
    public function CLS_GUIDE(){
        $this->objmysql=new CLS_MYSQL;
    }
    // property set value
    public function __set($proname,$value){
        if(!isset($this->pro[$proname])){
            echo ($proname.' is not member of CLS_GUIDE Class' );
            return;
        }
    ...
    public function getList($strwhere=""){
        $sql="SELECT * FROM tbl_guide WHERE 1=1 $strwhere";
        return $this->objmysql->Query($sql);
    }
    public function getCount($where=''){
        $objdata=new CLS_MYSQL;
        $sql="SELECT count(id) as count FROM `tbl_guide` WHERE 1=1 ".$where;
        $objdata->Query($sql);
    ...
    public function LoadConten($lagid=0){
        $sql="SELECT * FROM `tbl_guide` WHERE  isactive='1'";
        $objdata=new CLS_MYSQL();
        $objdata->Query($sql);
    ...
    public function getCatName($catid) {
        $sql="SELECT name FROM tbl_cate_guide WHERE cate_id='$catid'";
        $objdata=new CLS_MYSQL;
        $objdata->Query($sql);
    ...
    public  function getListCbRelateContent($getId='', $swhere='', $arrId=''){
        $sql="SELECT `id`,`title` FROM `tbl_guide` WHERE ".$swhere." `isactive`='1' ORDER BY `id` ASC";
        $objdata=new CLS_MYSQL();
        $objdata->Query($sql);
        if($objdata->Num_rows()<=0) return;
        while($rows=$objdata->Fetch_Assoc()){
    ...
    public function listTable($strwhere="",$page){
        ...
        $leng=MAX_ROWS_ADMIN;
        $sql="SELECT * FROM tbl_guide WHERE 1=1 $strwhere ORDER BY `id` DESC LIMIT $star,$leng";
        $objdata=new CLS_MYSQL();
    ...
    public function getOldArrListTags($con_id){
        $sql="SELECT * FROM `tbl_guide` WHERE `id`=$con_id ";
        $arrTags=array();
        $objdata=new CLS_MYSQL();
        $objdata->Query($sql);
        $row=$objdata->Fetch_Assoc();
    ...
    public function addNew(){
        $objdata=new CLS_MYSQL;
        $sql="INSERT INTO tbl_guide (`cate_id`,`code`,`thumb`,`cdate`,`mdate`,`author`,`ishot`,`isactive`,`title`,`intro`,`fulltext`,`meta_title`,`meta_key`,`meta_desc`) VALUES ";
        $sql.="('".$this->Cate_ID."','".$this->Code."','".$this->Thumb."','";
        $sql.=$this->Cdate."','".$this->Mdate."','".$this->Author."','".$this->isHot."','".$this->isActive."','".$this->Title."','".$this->Intro."','";
    ...
    public function Delete($ids){
        $objdata=new CLS_MYSQL;
        $sql="DELETE FROM `tbl_guide` WHERE `id` in ('$ids')";
        return $objdata->Query($sql);
    }
    public function setHot($ids){
        $sql="UPDATE `tbl_guide` SET `ishot`=if(`ishot`=1,0,1) WHERE `id` in ('$ids')";
        return $this->objmysql->Exec($sql);
    }
    public function setActive($ids,$status=''){
        $sql="UPDATE `tbl_guide` SET `isactive`='$status' WHERE `id` in ('$ids')";
        if($status=='')
            $sql="UPDATE `tbl_guide` SET `isactive`=if(`isactive`=1,0,1) WHERE `id` in ('$ids')";
        return $this->objmysql->Exec($sql);
    }
    function Order($arr_id,$arr_quan){
        $n=count($arr_id);
        for($i=0;$i<$n;$i++){
            $sql="UPDATE `tbl_guide` SET `order`='".$arr_quan[$i]."' WHERE `id` = '".$arr_id[$i]."' ";
            $this->objmysql->Exec($sql);
        }
    }
    /* combo box*/
    function getListCbItem($getId='', $swhere='', $arrId=''){
        $sql="SELECT id, name, code FROM tbl_guide WHERE ".$swhere." `isactive`='1' ORDER BY `name` ASC";
        $objdata=new CLS_MYSQL();
        $objdata->Query($sql);
        if($objdata->Num_rows()<=0) return;
        while($rows=$objdata->Fetch_Assoc()){
