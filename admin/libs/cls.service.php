...
class CLS_SERVICE{
    ...
    public function __set($proname,$value){
        if(!isset($this->pro[$proname])){
            echo ($proname.' is not member of CLS_SERVICE Class' );
            return;
        }
    ...
    public function getList($strwhere=""){
        $sql="SELECT * FROM tbl_service WHERE 1=1 $strwhere";
        return $this->objmysql->Query($sql);
    }
    public function getCount($where=''){
        $objdata=new CLS_MYSQL;
        $sql="SELECT count(id) as count FROM `tbl_service` WHERE 1=1 ".$where;
        $objdata->Query($sql);
    ...
    public function listTable($strwhere="",$page){
        ...
        $leng=MAX_ROWS_ADMIN;
        $sql="SELECT * FROM tbl_service WHERE 1=1 $strwhere ORDER BY `id` DESC LIMIT $star,$leng";
        $objdata=new CLS_MYSQL();
    ...
    public function addNew(){
        $objdata=new CLS_MYSQL;
        $sql="INSERT INTO tbl_service (`cate_id`,`code`,`thumb`,`cdate`,`mdate`,`author`,`ishot`,`isactive`,`title`,`intro`,`fulltext`,`meta_title`,`meta_key`,`meta_desc`) VALUES ";
    ...
    function Order($arr_id,$arr_quan){
        $n=count($arr_id);
        for($i=0;$i<$n;$i++){
            $sql="UPDATE `tbl_service` SET `order`='".$arr_quan[$i]."' WHERE `id` = '".$arr_id[$i]."' ";
